...
/* ------------------------------------------------------------------ */
/* Tunnel test area structures (based on CSIS satellite imagery)        */
/* ------------------------------------------------------------------ */

/** Tunnel portal: reinforced concrete portal structure with vehicle access */
function TunnelPortal({ def, m }: BuilderProps) {
  const [w, h, d] = def.size;
  return (
    <group>
      {/* Foundation slab */}
      ...
      {/* Main portal structure - arch opening */}
      <mesh material={m.concreteLight} castShadow receiveShadow position={[0, h / 2, 0]}>
        <boxGeometry args={[w, h, d]} />
      </mesh>
      {/* Arch cutout - dark interior */}
      <mesh material={m.interiorDark} position={[0, h * 0.35, d / 2 + 0.1]}>
        <boxGeometry args={[w * 0.65, h * 0.6, 0.5]} />
      </mesh>
      {/* Roof slab */}
      <mesh material={m.roofDark} castShadow position={[0, h + 0.15, 0]}>
        <boxGeometry args={[w + 1, 0.3, d + 1]} />
      </mesh>
      {/* Side walls extending outward */}
      <mesh material={m.concreteLight} castShadow position={[w / 2 + 1.5, h * 0.4, 0]}>
        <boxGeometry args={[2, h * 0.8, d - 2]} />
      </mesh>
      <mesh material={m.concreteLight} castShadow position={[-w / 2 - 1.5, h * 0.4, 0]}>
        <boxGeometry args={[2, h * 0.8, d - 2]} />
      </mesh>
      {/* Support columns at portal entrance */}
      {[-1, 1].map((side) => (
        <mesh key={`col${side}`} material={m.concrete} castShadow position={[side * (w * 0.4), h * 0.45, d / 2 - 1]}>
          <boxGeometry args={[1.2, h * 0.85, 1.2]} />
        </mesh>
      ))}
      {/* Warning light on roof */}
      <mesh material={m.beacon} position={[0, h + 0.5, 0]}>
        <sphereGeometry args={[0.25, 8, 8]} />
      </mesh>
      {/* Utility box on side wall */}
      <mesh material={m.metalDark} position={[w / 2 + 0.1, h * 0.3, d * 0.2]}>
...
/** Tunnel complex: central support building for the tunnel test area */
function TunnelComplex({ def, m }: BuilderProps) {
  const [w, h, d] = def.size;
  return (
    <group>
      {/* Main building */}
      ...
      <mesh material={m.concreteLight} castShadow receiveShadow position={[0, 0.6 + h / 2, 0]}>
        <boxGeometry args={[w, h, d]} />
      </mesh>
      {/* Roof */}
      <mesh material={m.roofDark} castShadow position={[0, 0.6 + h + 0.15, 0]}>
        <boxGeometry args={[w + 0.8, 0.3, d + 0.8]} />
      </mesh>
      {/* Control room wing - taller section */}
      <mesh material={m.concreteLight} castShadow position={[-w * 0.25, 0.6 + h * 1.2, -d * 0.3]}>
        <boxGeometry args={[w * 0.35, h * 0.6, d * 0.35]} />
      </mesh>
      {/* Control room windows */}
      <mesh material={m.glass} position={[-w * 0.25, 0.6 + h * 1.35, -d * 0.3 + d / 2 + 0.05]}>
        <boxGeometry args={[w * 0.28, h * 0.4, 0.1]} />
      </mesh>
      {/* Equipment annex */}
      <mesh material={m.corrugated} castShadow position={[w * 0.3, 0.6 + h * 0.45, d * 0.15]}>
        <boxGeometry args={[w * 0.3, h * 0.85, d * 0.5]} />
      </mesh>
      {/* Loading bay with roller door */}
      <mesh material={m.interiorDark} position={[w * 0.3, 0.6 + h * 0.35, d / 2 + 0.1]}>
        <boxGeometry args={[w * 0.22, h * 0.65, 0.3]} />
      </mesh>
      {/* Antenna mast on roof */}
      <mesh material={m.metalDark} castShadow position={[w * 0.15, 0.6 + h + 1.5, -d * 0.2]}>
        <cylinderGeometry args={[0.08, 0.12, 2.5, 6]} />
      </mesh>
      {/* Communication dishes */}
      <mesh material={m.whitePanel} castShadow position={[w * 0.2, 0.6 + h + 2.8, -d * 0.15]} rotation={[-0.5, 0.3, 0]}>
        <circleGeometry args={[0.8, 16]} />
      </mesh>
      <mesh material={m.whitePanel} castShadow position={[w * 0.1, 0.6 + h + 2.6, -d * 0.25]} rotation={[-0.4, -0.2, 0]}>
        <circleGeometry args={[0.6, 16]} />
      </mesh>
      {/* Perimeter fence section */}
      <mesh material={m.chainLink} position={[w / 2 + 1, h * 0.5, 0]}>
        <boxGeometry args={[0.1, h, d * 0.8]} />
      </mesh>
      <mesh material={m.chainLink} position={[-w / 2 - 1, h * 0.5, 0]}>
        <boxGeometry args={[0.1, h, d * 0.8]} />
      </mesh>
...
/** Decontamination station: vehicle decon facility */
function DeconStation({ def, m }: BuilderProps) {
  const [w, h, d] = def.size;
  return (
    <group>
      {/* Concrete pad */}
      <mesh material={m.concrete} receiveShadow position={[0, 0.1, 0]}>
        <boxGeometry args={[w + 2, 0.2, d + 4]} />
      </mesh>
      {/* Main canopy structure */}
      <mesh material={m.corrugatedWhite} castShadow receiveShadow position={[0, h / 2 + 0.5, 0]}>
        <boxGeometry args={[w, h, d]} />
      </mesh>
      {/* Support posts */}
      {[-1, 1].map((side) => (
        <mesh key={`post${side}`} material={m.metalDark} castShadow position={[side * (w / 2 - 0.5), h / 2 + 0.5, 0]}>
          <boxGeometry args={[0.3, h, 0.3]} />
        </mesh>
      ))}
      {/* Decon spray arch - tubular frame at entrance */}
      <mesh material={m.metalDark} position={[0, h * 0.5, d / 2 - 1]}>
        <boxGeometry args={[w * 0.8, h * 0.9, 0.15]} />
      </mesh>
      <mesh material={m.metalDark} position={[0, h * 0.5, -d / 2 + 1]}>
        <boxGeometry args={[w * 0.8, h * 0.9, 0.15]} />
      </mesh>
      {/* Side walls - partial for weather protection */}
      <mesh material={m.corrugatedTan} castShadow position={[w / 2 - 0.3, h * 0.45, 0]}>
        <boxGeometry args={[0.4, h * 0.85, d - 2]} />
      </mesh>
      <mesh material={m.corrugatedTan} castShadow position={[-w / 2 + 0.3, h * 0.45, 0]}>
        <boxGeometry args={[0.4, h * 0.85, d - 2]} />
      </mesh>
      {/* Control booth */}
      <mesh material={m.concreteLight} castShadow position={[w / 2 + 1, 1.5, d / 2 - 2]}>
        <boxGeometry args={[2.5, 2.8, 2.5]} />
      </mesh>
      <mesh material={m.glass} position={[w / 2 + 1, 2.2, d / 2 - 1.05]}>
        <boxGeometry args={[2, 1.2, 0.1]} />
      </mesh>
      {/* Water tank */}
      <mesh material={m.tankSteel} castShadow position={[-w / 2 + 0.5, h * 0.5, -d / 2 + 1.5]}>
        <cylinderGeometry args={[0.8, 0.8, h, 12]} />
      </mesh>
      {/* Drain grate */}
      <mesh material={m.metalDark} position={[0, 0.22, 0]}>
        <boxGeometry args={[w * 0.7, 0.05, d - 2]} />
      </mesh>
    </group>
  );
}

/** Monitoring post: seismic/atmospheric monitoring station */
function MonitoringPost({ def, m }: BuilderProps) {
  const [w, h] = def.size;
  return (
    <group>
      {/* Concrete foundation */}
      <mesh material={m.concrete} receiveShadow position={[0, 0.15, 0]}>
        <boxGeometry args={[w + 1, 0.3, w + 1]} />
      </mesh>
      {/* Equipment shelter */}
      <mesh material={m.concreteLight} castShadow receiveShadow position={[0, h / 2 + 0.3, 0]}>
        <boxGeometry args={[w, h, w]} />
      </mesh>
      {/* Roof */}
      <mesh material={m.roofDark} castShadow position={[0, h + 0.3 + 0.15, 0]}>
        <boxGeometry args={[w + 0.4, 0.3, w + 0.4]} />
      </mesh>
      {/* Antenna tower */}
      <mesh material={m.metalDark} castShadow position={[0, h + 0.3 + 4, 0]}>
        <cylinderGeometry args={[0.06, 0.08, 8, 6]} />
      </mesh>
      {/* Cross arms for antenna arrays */}
      <mesh material={m.metalDark} position={[0, h + 0.3 + 7, 0]}>
        <boxGeometry args={[3, 0.06, 0.06]} />
      </mesh>
      <mesh material={m.metalDark} position={[0, h + 0.3 + 6.5, 0]}>
        <boxGeometry args={[0.06, 0.06, 2.5]} />
      </mesh>
      {/* Ground equipment - seismometer housing */}
      <mesh material={m.concrete} castShadow position={[w / 2 + 1, 0.35, w / 2]}>
        <cylinderGeometry args={[0.6, 0.6, 0.7, 12]} />
      </mesh>
      {/* Solar panel for power */}
      <mesh material={m.solar} castShadow position={[-w / 2 - 0.5, 1.5, w / 2 - 1]} rotation={[-0.35, 0.2, 0]}>
        <boxGeometry args={[1.8, 0.1, 1.2]} />
      </mesh>
      {/* Fence perimeter */}
      {[-1, 1].map((side) => (
        <mesh key={`fence${side}`} material={m.chainLink} position={[side * (w / 2 + 0.8), 1, 0]}>
          <boxGeometry args={[0.1, 2, w]} />
        </mesh>
      ))}
    </group>
  );
...
/** Power station: dedicated power generation for tunnel operations */
function PowerStation({ def, m }: BuilderProps) {
  const [w, h, d] = def.size;
  return (
    <group>
      {/* Concrete pad */}
      <mesh material={m.concrete} receiveShadow position={[0, 0.2, 0]}>
        <boxGeometry args={[w + 3, 0.4, d + 3]} />
      </mesh>
      {/* Main generator building */}
      <mesh material={m.concreteLight} castShadow receiveShadow position={[0, h / 2 + 0.4, 0]}>
        <boxGeometry args={[w, h, d]} />
      </mesh>
      {/* Roof - slightly pitched */}
      <mesh material={m.roofDark} castShadow position={[0, h + 0.4 + 0.15, 0]}>
        <boxGeometry args={[w + 0.6, 0.3, d + 0.6]} />
      </mesh>
      {/* Generator units visible through ventilation */}
      <mesh material={m.metalDark} position={[w * 0.25, h * 0.35, d / 2 + 0.08]}>
        <boxGeometry args={[w * 0.35, h * 0.6, 0.2]} />
      </mesh>
      <mesh material={m.metalDark} position={[-w * 0.15, h * 0.35, d / 2 + 0.08]}>
        <boxGeometry args={[w * 0.25, h * 0.5, 0.2]} />
      </mesh>
      {/* Exhaust stack */}
      <mesh material={m.corrugated} castShadow position={[w / 2 - 1, h + 0.4 + 2.5, d / 2 - 1]}>
        <cylinderGeometry args={[0.5, 0.6, 5, 8]} />
      </mesh>
      {/* Fuel tank */}
      <mesh material={m.tankSteel} castShadow position={[-w / 2 - 1, h * 0.4, -d / 2 + 1.5]}>
        <cylinderGeometry args={[1.2, 1.2, h * 0.8, 12]} />
      </mesh>
      {/* Transformer unit */}
      <mesh material={m.metalDark} castShadow position={[w * 0.3, 1.2, -d / 2 + 1]}>
        <boxGeometry args={[2.5, 2.4, 2]} />
      </mesh>
      {/* Transformer bushings */}
      {[-0.4, 0.4].map((f) => (
        <mesh key={`bus${f}`} material={m.whitePanel} position={[w * 0.3 + f, 2.8, -d / 2 + 1]}>
          <cylinderGeometry args={[0.1, 0.14, 1, 8]} />
        </mesh>
      ))}
      {/* Power lines going out */}
      <mesh material={m.metalDark} position={[0, h + 0.4 + 0.5, d / 2 + 3]}>
        <boxGeometry args={[0.08, 0.08, 6]} />
      </mesh>
      {/* Cable trays */}
      <mesh material={m.metalDark} position={[w / 2 + 0.5, h * 0.6, 0]}>
        <boxGeometry args={[1, 0.1, d * 0.6]} />
      </mesh>
    </group>
  );
}
...
function StructureBody(props: BuilderProps) {
  switch (props.def.type) {
    ...
    case "tunnel-portal":
      return <TunnelPortal {...props} />;
    case "tunnel-complex":
      return <TunnelComplex {...props} />;
    case "decon-station":
      return <DeconStation {...props} />;
    case "monitoring-post":
      return <MonitoringPost {...props} />;
    case "power-station":
