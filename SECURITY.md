...
- Anything placed in a frontend build is public. Never add a credential to
  ...
  them.
- CI runs without repository secrets. Pull-request workflows are never granted
  write permissions or secret access, so a malicious pull request cannot
  exfiltrate anything by editing a workflow.
